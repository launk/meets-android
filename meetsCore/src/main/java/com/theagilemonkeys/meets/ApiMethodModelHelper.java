...
import com.theagilemonkeys.meets.utils.CacheDuration;
...
public class ApiMethodModelHelper<MODEL> implements ApiMethodModelHelperInterface<MODEL> {
    ...
    public Promise pushMethod(final ApiMethod method, final DelayedParams params) {
        ...
        if ( ! modelCacheEnable ) {
            method.setCacheDuration(CacheDuration.ALWAYS_EXPIRED);
        }

        if ( forceNextCacheEnable != null ) {
            method.setCacheDuration(forceNextCacheEnable ? method.getCacheDuration() : CacheDuration.ALWAYS_EXPIRED);
